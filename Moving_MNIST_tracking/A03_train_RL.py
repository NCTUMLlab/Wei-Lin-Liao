...
with tf.Session() as sess:
     ...
     with open("./tracking_results/RL_OPE","wb") as fp:
            pickle.dump(RL_ope_curve,fp)

     with open("./tracking_results/RL_box_pos","wb") as fp:
            pickle.dump(RL_box_pos,fp)

     with open("./tracking_results/RL_gd_pos","wb") as fp:
     ...
     plt.plot(precision_threshold_x,SL_ope_curve_y,'r',label='SL')
     plt.plot(precision_threshold_x,RL_ope_curve_y,'b',label='SL+RL')
     plt.xlabel('Overlap threshold')
     plt.ylabel('Success rate')
     plt.xlim(0,1)
     plt.legend()
     plt.savefig('./tracking_results/RL_OPE')
     plt.show()

     # Reinforcement learning
     save_model(sess,saver,'./model_final','RL_01')
