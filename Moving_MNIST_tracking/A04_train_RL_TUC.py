...
with tf.Session() as sess:
     ...
     for e in range(episodes):   
         ...
         ratio_2 = ratio_init_2*(e)/episodes   
           ...
           for s in range(search_step):
                 ...
                 if t > 0:
                    if t <= mem_size :
                       ...
                       if t == mem_size :
                          pre_z_mean, pre_z_std = tuc.dump_z_mean_std(state,temp_act)
                       ...
                       z_mean, z_std = tuc.dump_z_mean_std(state,temp_act)
                       intrinsic_reward_1 = KL_divergence(pre_z_mean,pre_z_std,z_mean,z_std)/mem_size
                       intrinsic_reward_2 = tuc.dump_regret(state,act)
                       pre_z_mean = z_mean
                       pre_z_std = z_std
                       # Update memory
                       states_mem.append(state)
                 ...
                 ep_rewards.append(ext_reward+ratio_init_1*intrinsic_reward_1+ratio_init_2*intrinsic_reward_2)
         ...
         pratical_returns = get_returns(ep_rewards)
         ...
         for ce in range(critic_epoch):
              tuc.train_critic(np.vstack(states), np.vstack(values), np.vstack(actions))   
     ...
     with open("./tracking_results/RL_TUC_OPE","wb") as fp:
            pickle.dump(RL_ope_curve,fp)

     with open("./tracking_results/RL_TUC_box_pos","wb") as fp:
            pickle.dump(RL_box_pos,fp)

     with open("./tracking_results/RL_TUC_gd_pos","wb") as fp:
     ...
     plt.plot(precision_threshold_x,SL_ope_curve_y,'r',label='SL',linewidth=5)
     plt.plot(precision_threshold_x,pure_RL_ope_curve_y,'b',label='SL+RL',linewidth=5)
     plt.plot(precision_threshold_x,RL_ope_curve_y,'g',label='SL+RL(TUC)',linewidth=5)
     plt.xlabel('Overlap threshold')
     plt.ylabel('Success rate')
     plt.xlim(0,1)
     plt.legend()
     plt.savefig('./tracking_results/RL_TUC_OPE')
     plt.show()
     
     save_model(sess,saver,'./model_final','RL_TUC_01')
