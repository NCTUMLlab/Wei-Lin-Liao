...
class PG:
      def __init__(self,
          ...
          self.EP_states, self.EP_actions, self.EP_values = [], [], []       
          self.decay = decay          
          self.agent_scope = net_name + '_agent'
          with tf.variable_scope(self.agent_scope, reuse = False):
               ...
               self.layer_1 = tf.layers.dense(inputs = self.states ,
                                              units = 3,
               ...
               self.all_act = tf.layers.dense(inputs = self.layer_1,
                                              units = action_dim,
                                              activation = None,
                                              kernel_initializer = tf.random_normal_initializer(mean=0, stddev=0.03),
                                              bias_initializer = tf.constant_initializer(0.1),
               ...
               self.all_act_prob = tf.nn.softmax(self.all_act, name = '_act_prob')
               self.agent_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope = self.agent_scope)
             
               
          with tf.variable_scope(net_name + '_REINFORCE_loss', reuse = False):
               self.neg_log_prob = neg_log_prob = tf.reduce_sum(-tf.log(self.all_act_prob)*tf.one_hot(self.actions, action_dim), axis=1)
               self.REINFORCE_loss = tf.reduce_mean(self.neg_log_prob*self.values)
               self.REINFORCE_optimizer = tf.train.AdamOptimizer(learning_rate).minimize(self.REINFORCE_loss, var_list = self.agent_vars)
      ...
      def agent_choose_action(self, state):
          prob_weights = self.sess.run(self.all_act_prob, feed_dict={self.states: state[np.newaxis, :]})
          action = np.random.choice(range(prob_weights.shape[1]), p=prob_weights.ravel())  # select action w.r.t the actions prob
          return action
          
      def agent_store_transition(self, state, action, reward):
          self.EP_states.append(state)
          self.EP_actions.append(action)
          self.EP_values.append(reward)
          
      def agent_get_values(self):
       
          discounted_values = np.zeros_like(self.EP_values)
          running_add = 0.
          for t in reversed(range(0, len(self.EP_values))):
              running_add = running_add * self.decay + self.EP_values[t]
      ...
      def agent_get_actions(self, state):
      ...
      def agent_REINFORCE(self):          

          self.discounted_ep_rs_norm = self.agent_get_values()

          
          self.sess.run(self.REINFORCE_optimizer, feed_dict={
                        self.states : np.vstack(self.EP_states),  # shape=[None, n_obs]
                        self.actions : np.array(self.EP_actions),  # shape=[None, ]
                        self.values : self.agent_get_values(),  # shape=[None, ]
                       })
      ...
      def __init__(self,
          ...
          with tf.variable_scope(self.net_name, reuse = False): 
               ...
               self.W1_mean = tf.get_variable(self.net_name+'_W1_mean',
                                      initializer = tf.truncated_normal([state_dim+action_dim,3],stddev = self.stddev))  
               self.W1_logstd = tf.get_variable(self.net_name+'_W1_logstd',
                                      initializer = tf.truncated_normal([state_dim+action_dim,3],stddev = self.stddev)) 
                                      
               self.b1_mean = tf.get_variable(self.net_name+'_b1_mean',
                                      initializer = tf.truncated_normal([3],stddev = self.stddev))  
               self.b1_logstd = tf.get_variable(self.net_name+'_b1_logstd',
                                      initializer = tf.truncated_normal([3],stddev = self.stddev))

               self.W1_noise = tf.random_normal([state_dim+action_dim,3], mean = 0., stddev = self.stddev_prior) 
               self.b1_noise = tf.random_normal([3], mean = 0., stddev = self.stddev_prior)               
          
               self.W2_mean = tf.get_variable(self.net_name+'_W2_mean',
                                      initializer = tf.truncated_normal([3,state_dim],stddev = self.stddev))  
               ...
               self.W2_noise = tf.random_normal([3,state_dim], mean = 0., stddev = self.stddev_prior) 
...
class TUC:
      def __init__(self,
          ...
          with tf.variable_scope(self.tuc_scope, reuse = False):
                 ...
                 for m in range(action_dim):
                      self.actions_weight.append(tf.tile(self.split_actions[m],[1,hidden_dim]))
      ...
      def dump_regret(self, states, act_chosen):
          ...
          temp_states = np.tile(states[np.newaxis,:], [self.action_dim,1])
